...
namespace Watermelon
{
    public class LevelEditorWindow : LevelEditorBase
    {
        ...
        protected override void InitialiseVariables()
        {
            ...
            currentButtonsWidth = SavesYG.GetInt(PREFS_BUTTONS_WIDTH, BUTTONS_WIDTH);
        ...
        private void OpenLastActiveLevel()
        {
            if (!EditorApplication.isPlayingOrWillChangePlaymode)
            {
                ...
                {
                    if ((levelsSerializedProperty.arraySize > 0) && SavesYG.HasKey(PREFS_LEVEL))
                    {
                        int levelIndex = Mathf.Clamp(SavesYG.GetInt(PREFS_LEVEL, 0), 0, levelsSerializedProperty.arraySize - 1);
        ...
        {
            SavesYG.SetInt(PREFS_LEVEL, index);
        ...
        private void HandleChangingButtonWidth()
        {
            ...
            if (buttonSeparatorIsDragged)
            {
                if (Event.current.type == EventType.MouseUp)
                {
                    ...
                    SavesYG.SetInt(PREFS_BUTTONS_WIDTH, currentButtonsWidth);
                    YandexGame.savesData.Save();
                    Event.current.Use();
                }
                else if (Event.current.type == EventType.MouseDrag)
